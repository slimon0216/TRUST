// graph.h
// Graph format: Json based format: [src_id, src_weigh,[[connected_ver_0,
// edge_weight],[connected_ver_1, edge_weight],[connected_ver_2, edge_weight]]]
// Storage format:
// struct{
//		int: src_ver
...
public:
  ...
  int *valid;
  // dynaic scheduling
  index_t ChunkNum; // = roof(upperEdgeCount/BufferSize), the number of chunks
                    // for workload edge list
  ...
  index_t
      *ds_help; // ds_last[P]; initiate with the last chunk it has to work with
  index_t
      *ds_last; // ds_last[P]; initiate with the last chunk it has to work with
