...
struct arguments Triangle_count(int rank, char name[100], struct arguments args,
  ...
  HRR(cudaMemcpy(d_beg_pos_HT, graph_HT->beg_pos,
                 sizeof(index_t) * (vertex_count + 1), cudaMemcpyHostToDevice));
  HRR(cudaMemcpy(d_adj_list_HT, graph_HT->adj_list,
                 sizeof(vertex_t) * edge_count, cudaMemcpyHostToDevice));
  ...
  HRR(cudaMalloc((void **)&d_beg_pos_intersection,
                 sizeof(index_t) * (vertex_count + 1)));
  HRR(cudaMalloc((void **)&d_adj_list_intersection,
                 sizeof(vertex_t) * (edge_count)));

  HRR(cudaMemcpy(d_beg_pos_intersection, graph_intersection->beg_pos,
                 sizeof(index_t) * (vertex_count + 1), cudaMemcpyHostToDevice));
  HRR(cudaMemcpy(d_adj_list_intersection, graph_intersection->adj_list,
                 sizeof(vertex_t) * edge_count, cudaMemcpyHostToDevice));
  ...
  HRR(cudaMalloc((void **)&d_beg_pos_neighbor,
                 sizeof(index_t) * (vertex_count + 1)));
  HRR(cudaMalloc((void **)&d_adj_list_neighbor,
                 sizeof(vertex_t) * (edge_count)));

  HRR(cudaMemcpy(d_beg_pos_neighbor, graph_neighbor->beg_pos,
                 sizeof(index_t) * (vertex_count + 1), cudaMemcpyHostToDevice));
  HRR(cudaMemcpy(d_adj_list_neighbor, graph_neighbor->adj_list,
                 sizeof(vertex_t) * edge_count, cudaMemcpyHostToDevice));
  // cout<<"total:"<<total<<endl;

  if (1) {
    HRR(cudaMalloc((void **)&GLOBAL_COUNT, sizeof(unsigned long long) * 10));
    HRR(cudaMalloc((void **)&g_gettime,
                   sizeof(unsigned long long) * (vertex_count + 1)));
    HRR(cudaMalloc((void **)&g_maxcollision,
                   sizeof(unsigned long long) * (vertex_count + 1)));
    ...
    HRR(cudaMemcpy(g_vertexmap, vertexmap, sizeof(int) * (nothreadfirstvertex),
  ...
  HRR(cudaMemcpy(counter, GLOBAL_COUNT, sizeof(unsigned long long) * 10,
                 cudaMemcpyDeviceToHost));
  // unsigned long long *gettime= new unsigned long long[vertex_count+1];
  // HRR(cudaMemcpy(gettime,g_gettime,sizeof(unsigned long
  // long)*(vertex_count+1), cudaMemcpyDeviceToHost)); unsigned long long
  // *maxcollision= new unsigned long long[vertex_count+1];
  // HRR(cudaMemcpy(maxcollision,g_maxcollision,sizeof(unsigned long
  // long)*(vertex_count+1), cudaMemcpyDeviceToHost));
