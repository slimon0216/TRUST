...
int main(int argc, char *argv[]) {
  ...
  //[For p2p08]:
  // int stat[16]=
  // {1313,2451,3704,4682,6005,7118,8562,9878,11177,12506,13800,15148,16410,17716,19200,20777};
  // For Facebook combined:
  // int
  // stat[16]={6660,12003,17400,23208,29515,34223,38163,41771,45113,48885,52745,56454,59673,65882,74390,88234};
  MPI_Status status;
  ...
  while (myrank < total_rank) {
    ...
    args = Triangle_count(myrank, argv[1], args, total_rank, N_THREADS,
                          N_BLOCKS, chunk_size, partition_num);
  ...
  if (myrank % numtasks == 0) {
    printf("\n%s,triangle:%lld,%f,%f\n", argv[1], global_sum, global_min_time,
           global_max_time);
  }
  MPI_Finalize();
  return 0;
