...
int endofprocess = -1;
bool cmp1(edge_list a, edge_list b) { return a.edge.size() < b.edge.size(); }
bool cmp2(edge_list a, edge_list b) { return a.edge.size() > b.edge.size(); }
...
void partition(int k) {
  ...
  for (int i = 0; i < vertex_count; i++) {
    ...
    for (int j = 0; j < k; j++)
      count[j] = 0;
    int ii = i % k;
    if (endofprocess == -1 && beg_pos[i + 1] - beg_pos[i] < 2)
      endofprocess = i;
...
int mapvertex(string foldname, int k) {
  ...
  for (int i = 0; i < endofprocess / k + 1; i++) {
    ...
    if (firstwarpvertex == -1 && ver[i].degree <= bounder)
      firstwarpvertex = i;
    if (firstnothreadvertex == -1 && ver[i].degree < 1) {
      firstnothreadvertex = i;
      break;
    }
  ...
  if (firstnothreadvertex == -1)
    firstnothreadvertex = endofprocess / k;
